...
public class Floating_eight extends Service {


	private WindowManager windowManager8;
    ...
    public int onStartCommand(Intent intent, int flags, int startId){
       ...
       PackageManager manager = this.getPackageManager();
		...
		windowManager8 = (WindowManager) getSystemService(WINDOW_SERVICE);
		...
		params.x = 20+ MainScope.E;
		params.y = 120+ MainScope.E;
		params.windowAnimations = android.R.style.Animation_Dialog;

		windowManager8.addView(chatHead, params);
			...
			chatHead.setOnTouchListener(new View.OnTouchListener() {
				...
				@Override public boolean onTouch(View v, MotionEvent event) {
					...
					case MotionEvent.ACTION_MOVE:
						...
						try{
							windowManager8.updateViewLayout(chatHead, paramsF);
						}
						catch (IllegalArgumentException e){
							windowManager8.addView(chatHead, paramsF);
						}
						break;
					}
					return false;
	...
	public void onDestroy() {
		...
		if (chatHead.isShown()) {
			windowManager8.removeView(chatHead);
		}
		MainScope.eight = false;
		MainScope.eightS = false;
		System.out.println("Channel EIGHT Reset");
	}
