...
public class Floating_Bluetooth2 extends Service {


	private WindowManager windowManagerB2;
    ...
    public int onStartCommand(Intent intent, int flags, int startId){
       ...
       PackageManager manager = this.getPackageManager();
		...
		if(RecieverBluetooth.hide == true){
			trans = true;
		}
		...
		if(trans == true){
			new Handler().postDelayed(new Runnable() {
				@Override
				public void run() {
					appIcon.setAlpha(RecieverBluetooth.alpha);
				}
			}, 2000);
		}

		windowManagerB2 = (WindowManager) getSystemService(WINDOW_SERVICE);
		...
		windowManagerB2.addView(chatHead, params);
		...
		try {
			chatHead.setOnTouchListener(new View.OnTouchListener() {
				...
				public boolean onTouch(View v, MotionEvent event) {
					if(trans == true){
						appIcon.setAlpha(RecieverBluetooth.opacity);
						new Handler().postDelayed(new Runnable() {
							@Override
							public void run() {
								appIcon.setAlpha(RecieverBluetooth.alpha);
					...
					case MotionEvent.ACTION_MOVE:
						...
						windowManagerB2.updateViewLayout(chatHead, paramsF);
						break;
					}
					return false;
				}
			...
			chatHead.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					if(RecieverBluetooth.blue2 == false){
						stopService(new Intent(getApplicationContext(), Floating_Bluetooth2.class));
						run = false;
	...
	public void onCreate() {
		...
		System.out.println("Floating_Bluetooth2");
	}
	...
	public void onDestroy() {
		...
		if (chatHead.isShown()) {
			windowManagerB2.removeView(chatHead);

			RecieverBluetooth.blue2 = true;
			System.out.println("Channel Floating_Bluetooth2 Reset");
		}
	}
