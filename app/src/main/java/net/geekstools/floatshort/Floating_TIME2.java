...
public class Floating_TIME2 extends Service {
       ...
       PackageManager manager = this.getPackageManager();
		...
		if(trans == true){
			new Handler().postDelayed(new Runnable() {
				@Override
				public void run() {
					appIcon.setAlpha(TimeHelper2.alpha);
				}
			}, 2000);
		}

		windowManagerT2 = (WindowManager) getSystemService(WINDOW_SERVICE);
		...
		windowManagerT2.addView(chatHead, params);
		...
		try {
			chatHead.setOnTouchListener(new View.OnTouchListener() {
				...
				public boolean onTouch(View v, MotionEvent event) {
					if(trans == true){
						appIcon.setAlpha(TimeHelper2.opacity);
						new Handler().postDelayed(new Runnable() {
							@Override
							public void run() {
								appIcon.setAlpha(TimeHelper2.alpha);
					...
					case MotionEvent.ACTION_MOVE:
						...
						windowManagerT2.updateViewLayout(chatHead, paramsF);
						break;
					}
					return false;
				}
			...
			chatHead.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					if(TimeHelper2.time2 == false){
						stopService(new Intent(getApplicationContext(), Floating_TIME2.class));
						run = false;
	...
	public void onDestroy() {
		...
		if (chatHead.isShown()) {
			windowManagerT2.removeView(chatHead);
