...
public class Floating_five extends Service {


	private WindowManager windowManager5;
    ...
    public int onStartCommand(Intent intent, int flags, int startId){
       ...
       PackageManager manager = this.getPackageManager();
		...
		windowManager5 = (WindowManager) getSystemService(WINDOW_SERVICE);
		...
		params.x = 10+ MainScope.E;
		params.y = 110+ MainScope.E;
		params.windowAnimations = android.R.style.Animation_Dialog;

		windowManager5.addView(chatHead, params);
			...
			chatHead.setOnTouchListener(new View.OnTouchListener() {
				...
				@Override public boolean onTouch(View v, MotionEvent event) {
					...
					case MotionEvent.ACTION_MOVE:
						...
						try{
							windowManager5.updateViewLayout(chatHead, paramsF);
						}
						catch (IllegalArgumentException e){
							windowManager5.addView(chatHead, paramsF);
						}
						break;
					}
					return false;
	...
	public void onDestroy() {
		...
		if (chatHead.isShown()) {
			windowManager5.removeView(chatHead);
		}
		MainScope.five = false;
		MainScope.fiveS = false;
		System.out.println("Channel FIVE Reset");
	}
