...
public class TimeScope extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(API > 20){
			...
			actBar.setTitle(Html.fromHtml("<font color='" + themColorString + "'>" + getResources().getString(R.string.time) + "</font>"));
			actBar.setSubtitle(Html.fromHtml("<font color='" + themColorString + "'>" + getResources().getString(R.string.time_desc) + "</font>"));
	...
	public void onResume(){
		...
		final File p1 = getApplicationContext().getFileStreamPath("time1");
		final File p2 = getApplicationContext().getFileStreamPath("time2");
		...
		if(p1.exists() || p2.exists()){
			...
			if(p1.exists() && p2.exists()){
				autoInfo.append(appName(READ("time1", getApplicationContext())) + "\n\n");
				autoInfo.append(appName(READ("time2", getApplicationContext())));
			}
			else if(p1.exists()){
				autoInfo.append(appName(READ("time1", getApplicationContext())));
			}
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		String time1 = "net.geekstools.floatshort.pro.timehelper1";
		String time2 = "net.geekstools.floatshort.pro.timehelper2";
		
		File timeFile = getApplicationContext().getFileStreamPath("time");
		//new File("/data/data/net.geekstools.floatshort.floatshort.PRO/files/time");
		if(timeFile.exists()){
			time =  time2;

			timeFile.delete();
			System.out.println("timeFile HERE! & DELETED");
		}
		else{
			time = time1;
			
			try {
				FileOutputStream fOut = openFileOutput("time", MODE_PRIVATE);
		...
		Intent t = new Intent(TimeScope.this, GetTime.class);
		t.putExtra("time", time);
		t.putExtra("pack", pack);
	...
	private class LoadApplications extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			listadaptor = new AdaptorScope(TimeScope.this, R.layout.snippet_list_row, applist);
		...
		protected void onPreExecute() {
			progress = ProgressDialog.show(TimeScope.this, null, "Loading Applications Info...");
