...
public class Floating_two extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId){
       ...
       PackageManager manager = this.getPackageManager();
		...
		windowManager2.addView(chatHead, params);
			...
			chatHead.setOnTouchListener(new View.OnTouchListener() {
				...
				@Override public boolean onTouch(View v, MotionEvent event) {
					...
					case MotionEvent.ACTION_MOVE:
						...
						try{
							windowManager2.updateViewLayout(chatHead, paramsF);
						}
						catch (IllegalArgumentException e){
							windowManager2.addView(chatHead, paramsF);
						}
						break;
					}
					return false;
	...
	public void onDestroy() {
		...
		if (chatHead.isShown()) {
			windowManager2.removeView(chatHead);
		}
		MainScope.two = false;
		MainScope.twoS = false;
		System.out.println("Channel TWO Reset");
	}
